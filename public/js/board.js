...
function updateNoteElement(el, note, opts = {}) {
...
document.getElementById('new-note-form').addEventListener('submit', async (e) => {
  ...
  const exists = state.board.notes.some((n) => n.id === note.id);
  if (!exists) state.board.notes.unshift(note);
  renderAll();
});
...
socket.on('note:created', (note) => {
  ...
  if (state.board.notes.some((n) => n.id === note.id)) return;
  state.board.notes.unshift(note);
  renderAll();
});

socket.on('note:updated', (note) => {
  if (!state.board) return;
  const idx = state.board.notes.findIndex((n) => n.id === note.id);
  if (idx === -1) {
    state.board.notes.unshift(note);
  } else {
    state.board.notes[idx] = note;
  }
  renderAll();
});
